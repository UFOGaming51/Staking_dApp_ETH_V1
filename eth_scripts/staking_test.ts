...
async function main() {
    ...
    let customAdminAddress = admin.address;
    ...
    const stakingHelper = await deployHelper.helper.deployDepositHelper(allUfoPools, allLpPools, goerli.ufoToken, goerli.lpToken);
    await stakingHelper.deployTransaction.wait(6);

    await run('verify:verify', {
        address: stakingHelper.address,
        constructorArguments: [allUfoPools, allLpPools, goerli.ufoToken, goerli.lpToken],
